...
def create_tmp_filter_file(iterate, filename='taubin-smoothing.mlx'):
...
def taubin_smoothing(in_file, out_file, iterate):
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        print("Example:")
        print(sys.argv[0] + " /home/myuser/mymesh.stl 5")
        exit(0)
    ...
    out_mesh = tmp_folder_name + filename + "_taubin_{iterate}".format(iterate=iterate) + ".stl"
    taubin_smoothing(in_mesh, out_mesh, iterate)

    print()
    print("Done Taubin Smoothing, find the files at: " + tmp_folder_name)
